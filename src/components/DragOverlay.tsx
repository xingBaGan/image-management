...
const DragOverlay: React.FC<DragOverlayProps> = ({ isDragging, importState }) => {
  ...
  return (
    <div className="flex fixed inset-0 z-10 justify-center items-center w-full h-full bg-black bg-opacity-30 backdrop-blur-sm">
      ...
      {importState === ImportStatus.Tagging ? <span className="text-lg text-white">tagging...</span> : importState === ImportStatus.Importing ? <span className="text-lg text-white">importing...</span> : <span className="text-lg text-white">Release to upload</span>}
    </div>
  );
};
